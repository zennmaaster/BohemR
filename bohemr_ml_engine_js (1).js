...
class VectorDatabase {
    ...
    async getUserSuccessfulRecs(userId) {
        ...
        for (const [key, batch] of this.recommendations) {
            if (key.startsWith(userId)) {
                // Find recommendations that got positive feedback
                for (const rec of batch.recommendations) {
                    const feedbackKey = userId + '_' + rec.id;
                    const feedback = this.feedback.get(feedbackKey);
                    
                    if (feedback && (feedback.feedbackType === 'love' || feedback.feedbackType === 'like')) {
                        userRecs.push({
                            ...rec,
                            feedback: feedback.feedbackType,
                            context: batch.contextVector
                        });
                    }
                }
            }
...
window.mlRecommendationEngine = new MLRecommendationEngine();